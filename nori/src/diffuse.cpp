...
public:
    Diffuse(const PropertyList &propList) : m_texture(nullptr){
        if(propList.has("albedo")) {
            PropertyList l;
            l.setColor("value", propList.getColor("albedo"));
            m_texture = static_cast<Texture<Color3f> *>(NoriObjectFactory::createInstance("constant_color", l));
        }
    }
    virtual ~Diffuse() {
        delete m_texture;
        ...
        switch (obj->getClassType()) {
            case ETexture:
                if(obj->getIdName() == "albedo") {
                    ...
                    m_texture = static_cast<Texture<Color3f> *>(obj);
                }
                else if (obj->getIdName() == "image") {
                    if (m_texture)
                        throw NoriException("There is already an image defined!");
                    m_texture = static_cast<Texture<Color3f>*>(obj);
                }
                else {
