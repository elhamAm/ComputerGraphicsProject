...
public:
    ...
    virtual T eval(const Point2f & uv) override {
        /* to be implemented */
        int s0 = (uv[0] +  m_delta[0])/ m_scale[0];
        int s1 = (uv[1] + m_delta[1])/ m_scale[1];

        //cout << "s0: " << s0%2 << "s1: " << s1%2 << endl;

        if( (abs(s0 % 2) == 0) && (abs(s1 % 2) == 0)){
            return m_value1;
        }
        else if((abs(s0 % 2) == 0) && (abs(s1 % 2) == 1)){
            return m_value2;
        }
        else if((abs(s0 % 2) == 1) && (abs(s1 % 2) == 1)){
            return m_value1;
        }
        else if((abs(s0 % 2) == 1) && (abs(s1 % 2) == 0)){
            return m_value2;
        }
    }
